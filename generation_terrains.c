...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
    ...
    int i,j;
    int obs_type;
    /*le centre de terrain doit etre libre*/
    T->tab[l/2][h/2] = LIBRE;
    for (i=0; i<l; i++){
      for (j=0; j<h; j++){
          if (i != (l/2) || j != (h/2)){
              nb_alea = rand()%100+1;
              /*generer au hasard un type d'obstacle*/
              obs_type = rand()%2;
              nb_alea = nb_alea/100;
              /*nb_alea est bien dans ce cadre*/
              if (nb_alea <= dObst) {
                  if (obs_type == 0){
                      T->tab[i][j] = ROCHER;
